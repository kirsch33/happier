...
`happier setup` chooses the relay before authentication, then signs in,
registers this machine, and reconciles automatic startup. The lower-level
`happier auth login` command remains available when the relay profile and
machine lifecycle are already configured.
...
4. In the app, use `/setup` → **Set up this computer** if you are on that machine, or set up the box over SSH if you are operating it remotely.
