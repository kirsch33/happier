...
description: Launch and follow subagents, Claude teams and provider-backed agent threads from one surface inside a session.
---

A session does not have to be one agent doing one thing. Work can be delegated —
to a review or plan run Happier manages, to teammates inside a Claude team, or
to task threads a provider spawns on its own — and the **Agents** surface is
where all of that becomes visible in one place.

<Callout type="warn">
  This is behind two switches, not one. Subagents are experimental and **off by
  default**, and experimental toggles are themselves hidden until the
  experiments switch at the top of **Settings → Features** is on. If you have
  turned Subagents on and still see nothing, the master switch is almost always
  why. See [Feature flags](/extras/feature-flags) for `execution.runs`.
</Callout>

In one session, you can have:

- **Happier-managed Subagents**: review, plan, and delegate runs started by Happier
...
There are three distinct ways to start more agent work:

- **Provider-native subagents** use the current Agent's own delegation or parallel-agent facility. They are the default for an ordinary request to delegate work when the Agent supports them.
- **Happier-managed execution runs and subagents** include review, plan, and delegate runs that Happier can list, route, approve, reopen, and control.
- **Independent Happier sessions** are separate top-level sessions. Creating one is not routine delegation.

Use a Happier-managed run when the user explicitly asks for a Happier-managed run, Happier delegation run, or Happier subagent; asks for another Agent backend, model provider, model, account, or service that the native facility cannot satisfy; or has enabled a custom rule that requires Happier. The presence of a discoverable Happier action does not by itself choose this route.

## Plan run vs plan mode

These are two different things that sound alike:
...
Use them when one of the Happier-routing conditions above applies and you want a bounded or long-lived run associated with a Happier session.
...
Backend selection uses the same current Agent catalog and launch options as the rest of Happier. Inspect the action spec and resolve its options at launch time for the accepted targets, models, configuration, accounts, connected services, and field names. Do not rely on a fixed provider/model recipe from documentation.

For an in-session agent or action start, omit `sessionId` to host the run in the invoking session. An explicit `sessionId` remains supported when you deliberately target another authorized session. To create a separate top-level session instead, use `session.spawn_new`; that is an independent workflow, not routine delegation.

You can observe a run with a bounded wait. A wait timeout is nonterminal: the run may still be active, and the timeout does not stop or fail it. Retry a start only when the returned provider evidence marks the cause as retryable, such as a typed transient rate limit; other backend failures need their reported remedy rather than an assumed retry or backend switch.
