...
Discoverability is not authorization to choose Happier-managed delegation. For an ordinary request to delegate work, use the current Agent's native subagent or parallel-agent facility when it has one. Discover and start a Happier-managed run only when the user explicitly asks for a Happier run, Happier delegation run, or Happier subagent; asks for another Agent backend, model provider, model, account, or service that the native facility cannot satisfy; or has enabled a custom rule that requires Happier.
...
- **Happier-managed execution runs** include Happier review, plan, and delegate subagents. Happier owns their launch, lifecycle, transcript access, and controls.
- **Independent Happier sessions** are top-level sessions created with `session.spawn_new`. They are not routine delegation and do not replace a subagent run.

### Starting and observing a Happier-managed run
