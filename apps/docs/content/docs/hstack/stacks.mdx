...
For a full explanation of the different folders/paths (`home` vs `workspace` vs `runtime` vs stack storage) and the exact env precedence rules, see: [[Paths, folders, and env precedence](/hstack/paths-and-env)](/hstack/paths-and-env).
