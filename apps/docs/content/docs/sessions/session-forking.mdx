...
An explicit Native choice is strict: if the provider-native fork is unavailable or fails, Happier
...
policy. Native forks stay in progress until the provider resolves them; Happier does not abandon one
