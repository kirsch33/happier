...
- **Resume session**: starting a new Happier session by resuming a session the agent already has (see [Continuing a session](/sessions/continuing-a-session))
...
- **Inactive + resumable + machine reachable**: you can send a message; Happier either attempts to resume it or leaves it queued according to your automatic-resume policy
- **Inactive + resumable + machine offline**: you can still send; Happier keeps the message in the Pending queue and starts it after reconnect only if that send has been authorized for processing when available
