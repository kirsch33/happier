...
- **The session's transcript is not hosted by Happier.** A [direct session](/sessions/continuing-a-session), where the Agent owns its own store, has no Happier conversation for another Agent to continue.
...
- **No reachable CLI can perform the operation, or its CLI is older than the feature.** Happier distinguishes the two where it can, and says "update or reconnect" when it genuinely cannot tell.
